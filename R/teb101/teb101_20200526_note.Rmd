...
model =train(churn~., data=trainset, method="rpart", trControl=control) #matrix: choose the index which choosing the model
...
# the result after 50 trees' training is similar, so 50 trees are enough
...
data(iris)
d = dist(iris[,-5],method = 'euclidean')
hc = hclust(d,method='ward.D2') #群集距離
...
#within.cluster.ss 組內距離平方和WSS(Within Cluster Sum of Squares) 越小越好
...
SW = sapply(nk,function(k){
  set.seed(22); cluster.stats(dist(iris_s),kmeans(iris_s,centers=k)$cluster)$avg.silwidth
})
