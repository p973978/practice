...
g+geom_bar()
g+geom_bar(fill='snow',color='black') #color 外框
```
...
library('esquisse')
esquisse::esquisser() #圖形化介面
```


### outliers
```{R}
scale(iris$Sepal.Width) #z-score
iris[abs(scale(iris$Sepal.Width)) > 3,] #select outliner

library(dplyr)
#找出非outliner的data
iris %>% filter(!abs(scale(iris$Sepal.Width)) > 3) #abs 絕對值，!abs否定
...
mice.data <- mice(data=iris,m = 3,method = "cart") # m = 3 產生3個樣本
...
library(arules)
library(arulesViz)
t1 = read.transactions('./data/transactions.txt',sep=',')
t1
inspect(t1)
image(t1)

t2 = read.csv('./data/transactions_matrix.txt',sep=',',header = T)
t2
t2 = as.matrix(t2)
