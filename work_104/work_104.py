...
header2["Referer"] = 'https://www.104.com.tw/job/' + List[0]['href'].split('job/')[1].split('?')[0] + '?jobsource=jolist_c_relevance'
    
url = 'https://www.104.com.tw/job/ajax/content/' + List[0]['href'].split('job/')[1].split('?')[0]
...
data = [{
    '公司' : json_data['data']['header']['custName'],
    '職缺' : json_data['data']['header']['jobName'],
    '工作內容' : json_data['data']['jobDetail']['jobDescription'],
    '福利制度' : json_data['data']['welfare']['welfare'],
    '薪水' : json_data['data']['jobDetail']['salary']
    }]
